      ...
      processRunner
        .Setup(runner =>
          runner.RunAsync(
            "cmd.exe",
            It.Is<string>(args =>
              args.Contains("--list-extensions", StringComparison.Ordinal)
              && args.Contains(cliPath, StringComparison.Ordinal)
            )
          )
        )
        .ReturnsAsync(
          new ProcessResult(0, "ms-vscode-remote.vscode-remote-extensionpack\r\nms-dotnettools.csharp", string.Empty)
        );
      ...
      processRunner
        .Setup(runner =>
          runner.RunAsync(
            "cmd.exe",
            It.Is<string>(args =>
              args.Contains("--install-extension", StringComparison.Ordinal)
              && args.Contains(cliPath, StringComparison.Ordinal)
            )
          )
        )
  ...
  public async Task RunCodeCliAsync_WhenCliCannotBeResolved_FallsBackToCodeOnPath()
  {
    ...
    processRunner
      .Setup(runner =>
        runner.RunAsync(
          "cmd.exe",
          It.Is<string>(args => args.Contains("/c code --list-extensions", StringComparison.Ordinal))
        )
      )
      .ReturnsAsync(new ProcessResult(0, "ms-dotnettools.csharp", string.Empty));
