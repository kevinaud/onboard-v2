...
public class InstallDockerDesktopStepTests
{
  ...
  public async Task ShouldExecuteAsync_WhenDockerDesktopDetected_ReturnsFalse()
  {
    processRunner
      .Setup(runner =>
        runner.RunAsync(
          "powershell",
          It.Is<string>(args => args.Contains("Docker Desktop.exe", StringComparison.Ordinal))
        )
      )
  ...
  public async Task ExecuteAsync_WhenWingetSucceeds_PrintsSuccessAndGuidance()
  {
    processRunner
      .Setup(runner =>
        runner.RunAsync(
          "winget",
          It.Is<string>(args => args.Contains("Docker.DockerDesktop", StringComparison.Ordinal))
        )
      )
    ...
    userInteraction.Setup(ui =>
      ui.WriteNormal(
        It.Is<string>(message => message.Contains(configuration.WslDistroName, StringComparison.OrdinalIgnoreCase))
      )
    );
  ...
  private InstallDockerDesktopStep CreateStep() =>
    new(processRunner.Object, userInteraction.Object, configuration, environmentRefresher.Object);
}
