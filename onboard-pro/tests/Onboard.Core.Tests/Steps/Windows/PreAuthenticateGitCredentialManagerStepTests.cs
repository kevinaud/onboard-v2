...
public class PreAuthenticateGitCredentialManagerStepTests
{
  ...
  public async Task ExecuteAsync_WhenLoginSucceeds_WritesSuccess()
  {
    ...
    try
    {
      ...
      processRunner
        .InSequence(sequence)
        .Setup(runner =>
          runner.RunAsync(cliPath, "auth login --hostname github.com --git-protocol https --web", false, true)
        )
        .ReturnsAsync(new ProcessResult(0, string.Empty, string.Empty));
  ...
  public async Task ExecuteAsync_WhenCliPathCached_UsesCachedPath()
  {
    ...
    try
    {
      ...
      processRunner
        .Setup(runner =>
          runner.RunAsync(tempFile, "auth login --hostname github.com --git-protocol https --web", false, true)
        )
        .ReturnsAsync(new ProcessResult(0, string.Empty, string.Empty));
  ...
  public async Task ExecuteAsync_WhenCliPathCannotBeResolved_FallsBackToGhOnPath()
  {
    ...
    processRunner
      .InSequence(sequence)
      .Setup(runner =>
        runner.RunAsync("gh", "auth login --hostname github.com --git-protocol https --web", false, true)
      )
      .ReturnsAsync(new ProcessResult(0, string.Empty, string.Empty));
