...
public class ConfigureDockerDesktopWslIntegrationStepTests
{
    ...
    public async Task ShouldExecuteAsync_WhenDistroAlreadyIntegrated_ReturnsFalse()
    {
        ...
        {
            ["IntegratedWslDistros"] = new JsonArray(TargetDistro),
        };
    ...
    public async Task ExecuteAsync_WhenIntegrationAdded_WritesSettingsAndRestarts()
    {
        ...
        fileSystem.Setup(fs => fs.WriteAllText(It.Is<string>(p => p.StartsWith(SettingsPath, StringComparison.Ordinal)), It.IsAny<string>()))
        ...
        processRunner
            .Setup(runner => runner.RunAsync("powershell", "-NoProfile -Command \"Start-Process -FilePath 'Docker Desktop' -Verb RunAs -ArgumentList '--shutdown'\"", false, true))
    ...
    public async Task ExecuteAsync_WhenParseFails_WarnsAndSkips()
    {
        ...
        userInteraction.Setup(ui => ui.WriteWarning(It.Is<string>(message => message.Contains("parse", StringComparison.OrdinalIgnoreCase))));
        ...
        processRunner.Verify(runner => runner.RunAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<bool>(), It.IsAny<bool>()), Times.Never);
    }
    ...
    public async Task ExecuteAsync_WhenAppDataMissing_Warns()
    {
        ...
        userInteraction.Setup(ui => ui.WriteWarning("Unable to locate the AppData folder. Configure Docker Desktop's WSL integration manually."));
    ...
    public async Task ExecuteAsync_WhenRestartFails_Warns()
    {
        ...
        userInteraction.Setup(ui => ui.WriteNormal("Restarting Docker Desktop to apply updated WSL integration..."));
        userInteraction.Setup(ui => ui.WriteWarning("Docker Desktop restart was requested but may not have completed. Restart Docker Desktop manually if required."));
