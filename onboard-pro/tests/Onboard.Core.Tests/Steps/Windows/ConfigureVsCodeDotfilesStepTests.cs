...
public class ConfigureVsCodeDotfilesStepTests
{
  ...
  public async Task ExecuteAsync_WhenUserProvidesCustomRepository_WritesSettings()
  {
    ...
    fileSystem
      .Setup(fs => fs.WriteAllText(SettingsPath, It.IsAny<string>()))
    ...
    userInteraction.Setup(ui => ui.WriteMarkdown(It.IsAny<string>())).Callback<string>(value => markdownPrompt = value);
    userInteraction.Setup(ui => ui.WriteSuccess("VS Code dotfiles configuration updated."));
    userInteraction.Setup(ui => ui.Ask(It.IsAny<string>(), It.IsAny<string?>())).Returns(() => responses.Dequeue());
  ...
  public async Task ExecuteAsync_WhenUserAcceptsDefaultRepository_WritesSettings()
  {
    ...
    userInteraction.Setup(ui =>
      ui.WriteMarkdown(It.Is<string>(value => value.Contains("`kevinaud/dotfiles`", StringComparison.Ordinal)))
    );
    userInteraction.Setup(ui => ui.WriteSuccess("VS Code dotfiles configuration updated."));
    userInteraction.Setup(ui => ui.Ask(It.IsAny<string>(), It.IsAny<string?>())).Returns(() => responses.Dequeue());
