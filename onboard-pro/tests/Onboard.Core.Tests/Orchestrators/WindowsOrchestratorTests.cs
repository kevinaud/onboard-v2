...
public class WindowsOrchestratorTests
{
  ...
  public async Task ExecuteAsync_SkipsStepsWhenAlreadyConfigured()
  {
    ...
    try
    {
      var processRunner = CreateProcessRunner(
        new Dictionary<(string FileName, string Arguments, bool RequestElevation, bool UseShellExecute), ProcessResult>
        {
          {
            (
              FileName: "dism.exe",
              Arguments: "/online /Get-FeatureInfo /FeatureName:Microsoft-Windows-Subsystem-Linux",
          ...
          },
          {
            (
              FileName: "dism.exe",
              Arguments: "/online /Get-FeatureInfo /FeatureName:VirtualMachinePlatform",
              RequestElevation: false,
              UseShellExecute: false
            ),
            Success("State : Enabled")
          ...
          {
            (FileName: "wsl.exe", Arguments: "-l -q", RequestElevation: false, UseShellExecute: false),
            Success("Ubuntu-22.04\r\n")
          },
          {
            (
              FileName: "wsl.exe",
              Arguments: "-d \"Ubuntu-22.04\" -- cat /etc/os-release",
              RequestElevation: false,
          ...
          {
            (FileName: "where", Arguments: "git.exe", RequestElevation: false, UseShellExecute: false),
            Success(gitPath)
          },
          {
            (FileName: "where", Arguments: "gh.exe", RequestElevation: false, UseShellExecute: false),
            Success(ghPath)
          },
          {
            (FileName: "where", Arguments: "code.cmd", RequestElevation: false, UseShellExecute: false),
            Success(codePath)
          },
          {
            (
              FileName: "powershell",
              Arguments: "-NoProfile -Command \"(Test-Path 'C:\\Program Files\\Docker\\Docker\\Docker Desktop.exe') -or (Test-Path (Join-Path $env:LOCALAPPDATA 'Programs\\Docker\\Docker Desktop.exe'))\"",
              RequestElevation: false,
              UseShellExecute: false
            ),
            Success("True")
          },
          {
            ...
            Success("ms-vscode-remote.vscode-remote-extensionpack")
          },
          {
            (
              FileName: "cmd.exe",
              Arguments: BuildCredentialProbeArguments(configuration),
              RequestElevation: false,
              UseShellExecute: false
            ),
          ...
          {
            (FileName: "git", Arguments: "config --global user.name", RequestElevation: false, UseShellExecute: false),
            Success("Test User")
          },
          {
            (FileName: "git", Arguments: "config --global user.email", RequestElevation: false, UseShellExecute: false),
      ...
      ui.Verify(
        x =>
          x.RunStatusAsync(It.IsAny<string>(), It.IsAny<Func<IStatusContext, Task>>(), It.IsAny<CancellationToken>()),
        Times.Exactly(10)
      );
      ...
      Assert.That(
        capturedSummary.All(result => string.Equals(result.SkipReason, "Already configured", StringComparison.Ordinal))
      );
      Assert.That(
        capturedSummary.Select(result => result.StepName).ToArray(),
        Is.EqualTo(
          new[]
          {
    ...
    var processRunner = CreateProcessRunner(
      new Dictionary<(string FileName, string Arguments, bool RequestElevation, bool UseShellExecute), ProcessResult>
      {
        ...
        },
        {
          (
            FileName: "cmd.exe",
            Arguments: "/c C:\\VSCode\\code.cmd --list-extensions",
            RequestElevation: false,
            UseShellExecute: false
          ),
          Success("ms-vscode-remote.vscode-remote-extensionpack")
  ...
  public async Task ExecuteAsync_WithDryRun_OnlyReportsSteps()
  {
      ...
      {
        {
          ...
          new ProcessResult(1, string.Empty, "WSL not enabled")
        },
        {
          (
            FileName: "dism.exe",
        ...
        {
          (FileName: "wsl.exe", Arguments: "-l -q", RequestElevation: false, UseShellExecute: false),
          new ProcessResult(1, string.Empty, "WSL not installed")
        },
        {
          (FileName: "where", Arguments: "git.exe", RequestElevation: false, UseShellExecute: false),
          new ProcessResult(1, string.Empty, "git not found")
        },
        {
          (FileName: "where", Arguments: "code.cmd", RequestElevation: false, UseShellExecute: false),
          new ProcessResult(1, string.Empty, "code not found")
        },
        {
          (FileName: "where", Arguments: "gh.exe", RequestElevation: false, UseShellExecute: false),
          new ProcessResult(1, string.Empty, "gh not found")
        },
        {
          (
            FileName: "powershell",
          ...
          new ProcessResult(1, string.Empty, "Docker not installed")
        },
        {
          (
            FileName: "cmd.exe",
    ...
    ui.Verify(
      x => x.RunStatusAsync(It.IsAny<string>(), It.IsAny<Func<IStatusContext, Task>>(), It.IsAny<CancellationToken>()),
      Times.Exactly(10)
    );
  ...
  public async Task ExecuteAsync_WhenInteractiveStepRuns_PromptsOutsideStatus()
  {
    ...
    ui.Setup(x =>
      ...
      .Returns(
        async (string _, Func<IStatusContext, Task> action, CancellationToken token) =>
        {
          isInsideStatus = true;
      ...
      var processRunner = CreateProcessRunner(
        new Dictionary<(string FileName, string Arguments, bool RequestElevation, bool UseShellExecute), ProcessResult>
        {
          ...
          },
          {
            (
              FileName: "git",
              Arguments: "config --global user.name \"Test User\"",
              RequestElevation: false,
              UseShellExecute: false
            ),
            Success(string.Empty)
          },
          {
            (
              FileName: "git",
              Arguments: "config --global user.email \"test@example.com\"",
              RequestElevation: false,
              UseShellExecute: false
            ),
            Success(string.Empty)
  ...
  )
  {
    ...
    var dockerIntegrationStep = new ConfigureDockerDesktopWslIntegrationStep(
      processRunner,
      ui.Object,
      fileSystem,
      configuration,
  ...
  private static Mock<IUserInteraction> CreateUserInteractionMock(
    Action<IReadOnlyCollection<StepResult>> summaryCallback
  )
  {
    var ui = new Mock<IUserInteraction>(MockBehavior.Loose);
    ...
    ui.Setup(x =>
      ...
      .Returns(
        (string _, Func<IStatusContext, Task> action, CancellationToken token) =>
          action(new TestStatusContext(ui.Object, token))
      );

    ui.Setup(x => x.ShowSummary(It.IsAny<IReadOnlyCollection<StepResult>>())).Callback(summaryCallback);

    return ui;
  }

  private static FakeProcessRunner CreateProcessRunner(
    IReadOnlyDictionary<
      (string FileName, string Arguments, bool RequestElevation, bool UseShellExecute),
      ProcessResult
    > responses
  ) => new(responses);
  ...
  private sealed class FakeProcessRunner : IProcessRunner
  {
    ...
    )
    {
      ...
      )
      {
        string key = BuildKey(
          entry.Key.FileName,
          entry.Key.Arguments,
          entry.Key.RequestElevation,
          entry.Key.UseShellExecute
        );
    ...
    public Task<ProcessResult> RunAsync(string fileName, string arguments) =>
      this.RunAsync(fileName, arguments, requestElevation: false, useShellExecute: false);

    public Task<ProcessResult> RunAsync(string fileName, string arguments, bool requestElevation) =>
      this.RunAsync(fileName, arguments, requestElevation, useShellExecute: false);
    ...
    public Task<ProcessResult> RunAsync(string fileName, string arguments, bool requestElevation, bool useShellExecute)
    {
      ...
      if (!this.responses.TryGetValue(lookupKey, out ProcessResult? result) || result is null)
      {
        throw new InvalidOperationException(
          $"Unexpected command: {fileName} {arguments} (elevated: {requestElevation}, shell: {useShellExecute})"
        );
      }
