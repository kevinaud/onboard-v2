...
public class ProcessRunnerTests
{
  ...
  public async Task RunAsync_WhenVerbose_WritesDebugMessages()
  {
    ...
    Assert.That(
      userInteraction.DebugMessages.Any(m => m.StartsWith("Completed with exit code", StringComparison.Ordinal)),
      Is.True
    );
  }
  ...
  public async Task RunAsync_WithDryRunAndVerbose_ReturnsSuccessAndLogsDryRun()
  {
    ...
    var runner = new ProcessRunner(
      NullLogger<ProcessRunner>.Instance,
      new ExecutionOptions(IsDryRun: true, IsVerbose: true),
      userInteraction
    );
    ...
    Assert.Multiple(() =>
    {
      ...
      Assert.That(
        userInteraction.DebugMessages,
        Is.EquivalentTo(new[] { "[DRY-RUN] Would execute: nonexistent --arg" })
      );
    });
  }
