...
public class ConfigureGitUserStep : IOnboardingStep, IInteractiveOnboardingStep
{
  ...
  public async Task<bool> ShouldExecuteAsync()
  {
    ...
    return !nameResult.IsSuccess
      ...
      || string.IsNullOrWhiteSpace(emailResult.StandardOutput);
  }
  ...
  public async Task ExecuteAsync()
  {
    ...
    var nameSetResult = await processRunner
      .RunAsync("git", $"config --global user.name \"{name}\"")
    ...
    var emailSetResult = await processRunner
      .RunAsync("git", $"config --global user.email \"{email}\"")
      .ConfigureAwait(false);
