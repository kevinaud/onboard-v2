...
public class EnableWslFeaturesStep : IOnboardingStep
{
  ...
  public async Task ExecuteAsync()
  {
    ...
    if (!readiness.CanQueryDistributions)
    {
      ...
      throw new InvalidOperationException(
        "WSL could not enumerate distributions. Install the required distro and rerun the onboarding tool."
      );
    }
  ...
  private async Task<WslReadiness> EvaluateReadinessAsync()
  {
    ...
    bool hasTarget = distributionNames.Any(name =>
      string.Equals(name, this.configuration.WslDistroName, StringComparison.OrdinalIgnoreCase)
    );
  ...
  private void HandleMissingDistribution(IReadOnlyCollection<string> detectedDistributions)
  {
    ...
    if (detectedDistributions.Count == 0)
    {
      ...
      PromptForManualInstall();
      throw new InvalidOperationException(
        $"Install '{this.configuration.WslDistroImage}' and rerun the onboarding tool."
      );
    }
    ...
    this.userInteraction.WriteNormal(
      $"Rename it so future runs detect it: wsl.exe --rename \"{selection}\" \"{targetName}\""
    );
    this.userInteraction.WriteNormal(
      $"Alternatively install the official image: wsl.exe --install -d {this.configuration.WslDistroImage}"
    );

    throw new InvalidOperationException(
      $"Rename the selected distribution to '{targetName}' (or install it) and rerun the onboarding tool."
    );
  }
  ...
  private void PromptForManualInstall()
  {
    ...
    this.userInteraction.WriteNormal(
      $"  3. Launch {this.configuration.WslDistroName} once so the user account is created, then rerun this onboarding tool."
    );
  }
  ...
  private string? PromptForDistributionSelection(IReadOnlyCollection<string> detectedDistributions)
  {
    ...
    while (true)
    {
      string response = this.userInteraction.Ask(
        "Enter the name of the Ubuntu 22.04 distribution from the list above (or type NONE):",
        "NONE"
      );
      ...
      string? match = detectedDistributions.FirstOrDefault(name =>
      ...
      this.userInteraction.WriteWarning(
        $"'{response}' did not match any detected distribution. Try again or type NONE."
      );
    }
  }
  ...
  {
    private WslReadiness(
      bool canQueryDistributions,
      bool hasTargetDistribution,
      IReadOnlyCollection<string> detectedDistributions,
      bool isInitialized
    )
    {
    ...
    public static WslReadiness Create(
      bool canQueryDistributions,
      bool hasTargetDistribution,
      IReadOnlyCollection<string> detectedDistributions
