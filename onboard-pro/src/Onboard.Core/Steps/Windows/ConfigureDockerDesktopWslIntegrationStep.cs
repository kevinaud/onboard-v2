...
{
  private const string RestartCommand =
  ...
  public async Task ExecuteAsync()
  {
    ...
    if (cachedState.Status == SettingsStatus.MissingAppData)
    {
      userInteraction.WriteWarning(
        "Unable to locate the AppData folder. Configure Docker Desktop's WSL integration manually."
      );
      return;
    }
    ...
    {
      userInteraction.WriteWarning(
        $"Failed to parse Docker Desktop settings: {cachedState.FailureMessage}. Review the file manually before rerunning onboarding."
      );
      return;
    }
    ...
    {
      userInteraction.WriteWarning(
        "No WSL distribution was detected. Run the WSL prerequisite step before configuring Docker Desktop."
      );
      return;
    }
    ...
    var restartResult = await processRunner
      .RunAsync("powershell", RestartCommand, requestElevation: false, useShellExecute: true)
  ...
  {
    public static SettingsState Ready(
      string targetDistro,
      string directory,
      string path,
      JsonObject settings,
